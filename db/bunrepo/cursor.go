...
//
// Wire-compatible with db/repo's cursor format, so a service migrating from GORM to
// bun does not invalidate cursors already held by clients.
func encodeCursor(values map[string]any) (string, error) {
	raw := make(map[string]json.RawMessage, len(values))
	for k, v := range values {
		b, err := json.Marshal(v)
		if err != nil {
			return "", fmt.Errorf("bunrepo: encode cursor %s: %w", k, err)
		}
		raw[k] = b
	}
	b, err := json.Marshal(cursorPayload{Values: raw})
	if err != nil {
		return "", fmt.Errorf("bunrepo: encode cursor: %w", err)
	}
...
func decodeCursor(cursor string) (map[string]json.RawMessage, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("bunrepo: decode cursor: %w", err)
	}
	var p cursorPayload
...
// as a bun WHERE argument. Uses json.Number to avoid float64 truncation for large
// int64 timestamps or numeric primary keys (JSON numbers default to float64 otherwise).
//
// The json.Number is converted back to a concrete int64/float64 rather than passed
// through: json.Number is a string type, so a driver would bind it as text and a
// bigint comparison would fail.
...
// Columns failing validColumn are dropped — they reach ORDER BY as raw SQL.
